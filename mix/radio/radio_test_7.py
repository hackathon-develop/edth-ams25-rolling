...
def read_crsf_frame():
    ...
    while True:
        ...
        if byte == b'\xC8':  # Start of CRSF frame
            length_byte = ser.read()
            if not length_byte:
                continue  # Skip if no length byte received
            length = length_byte[0]
            payload = ser.read(length)

            # Debugging output to inspect frame data
            print(f"Received CRSF frame with length {length}, payload: {payload.hex()}")

            # Validate the payload length and structure
            if len(payload) >= 22 and payload[0] == 0x16:  # RC channels frame
                data = payload[2:22]  # The channels data starts from index 2
                try:
                ...
                print("Warning: Incomplete or malformed CRSF frame received.")
                continue  # Skip incomplete or malformed frames
